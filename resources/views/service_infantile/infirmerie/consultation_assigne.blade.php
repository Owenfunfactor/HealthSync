    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Patients assignés</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
                    ...
                    <tbody>
                        @foreach ($consultations as $consultation)
                            @php
                              ...
                              $suivie = $consultation->suivie;
                              }
                            @endphp
                            <tr>
                                <td>{{ $consultation->id }}</td>
                                <td>{{ $consultation->child->nom }}</td>
                                @foreach ($consultation->users->where('type_compte', 'Traitant') as $user)
                                <td>{{ $user->name }}</td>
                                @endforeach
                                <td>{{ $consultation->child->created_at->format('d/m/Y') }}</td>
                                <td>
                                  <div class="d-flex gap-2 w-100 justify-content-end">
                                      <a class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#monModal{{ $consultation->child->id }}"><i class="fa fa-eye"></i></a>
                                      @if ($consultation->suivie == null)
                                        <a href="{{ route('suivie.pediatrie', $consultation->id) }}" class="btn btn-primary"><i class="fa-solid fa-pen"></i></a>
                                      @else
                                        @if ($suivie->demandeModif == null )
                                          <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modification{{ $consultation->id }}"><i class="fa fa-hand"></i></a>
                                        @elseif($suivie->demandeModif->status == 1)
                                          <a href="{{ route('Service_Infantile.suivie.edit', ['suivie' => $suivie->id]) }}" class="btn btn-warning"><i class="fa-solid fa-pen"></i></a>  
                                        @endif
                                      @endif     
                                      <!-- Début modal -->
                                      <div class="modal fade" id="monModal{{ $consultation->child->id }}" tabindex="-1" aria-labelledby="monModal{{ $consultation->child->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-lg">
                                          <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                              ...
                                              <h4 class="mt-3 text-center">Infos Consultations</h4>
                                                @foreach($consultation->users->where('type_compte', 'Traitant') as $user)
                                      ...
                                      <!-- Fin modal -->
                                      @include('service_infantile.infirmerie.demande_modification')
                                      <!-- Fin modal demande modification-->
                                  </div>
                                </td>
                            </tr>
                        @endforeach
